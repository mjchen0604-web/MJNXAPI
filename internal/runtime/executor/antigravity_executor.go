...
func (e *AntigravityExecutor) refreshToken(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	clientID, clientSecret := config.ResolveAntigravityOAuthClient(e.cfg)
	if clientID == "" || clientSecret == "" {
		return auth, statusErr{code: http.StatusBadRequest, msg: "antigravity oauth client not configured"}
	}
