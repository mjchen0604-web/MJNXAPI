...
func prepareGeminiCLITokenSource(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth) (oauth2.TokenSource, map[string]any, error) {
	...
	clientID, clientSecret := config.ResolveGeminiOAuthClient(cfg)
	if clientID == "" || clientSecret == "" {
		return nil, nil, fmt.Errorf("gemini oauth client is not configured; set oauth.gemini.client-id/client-secret or GEMINI_OAUTH_CLIENT_ID/SECRET")
	}
