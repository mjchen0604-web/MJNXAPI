...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	clientID, clientSecret := config.ResolveGeminiOAuthClient(h.cfg)
	if clientID == "" || clientSecret == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "gemini oauth client not configured"})
		return
	}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	clientID, clientSecret := config.ResolveAntigravityOAuthClient(h.cfg)
	if clientID == "" || clientSecret == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "antigravity oauth client not configured"})
		return
	}
