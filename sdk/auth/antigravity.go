...
func (AntigravityAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	clientID, clientSecret := config.ResolveAntigravityOAuthClient(cfg)
	if clientID == "" || clientSecret == "" {
		return nil, fmt.Errorf("antigravity oauth client is not configured; set oauth.antigravity.client-id/client-secret or ANTIGRAVITY_OAUTH_CLIENT_ID/SECRET")
	}
	...
	authURL := buildAntigravityAuthURL(redirectURI, state, clientID)
...
waitForCallback:
	...
	if cbRes.Code == "" {
		...
		tokenResp, errToken := exchangeAntigravityCode(ctx, cbRes.Code, redirectURI, clientID, clientSecret, httpClient)
	if errToken != nil {
...
func exchangeAntigravityCode(ctx context.Context, code, redirectURI, clientID, clientSecret string, httpClient *http.Client) (*antigravityTokenResponse, error) {
	data := url.Values{}
	data.Set("code", code)
...
func buildAntigravityAuthURL(redirectURI, state, clientID string) string {
	params := url.Values{}
